...
public class CollectJoiningTest {
	...
	public static void main(String[] args) {
		...
		// Join all the names without space

		String delimitedNames = persons.stream().map(Person::getName).collect(Collectors.joining(", "));
		// Join all the names separated by ,

		String prefixedNames = persons.stream().map(Person::getName)
				.collect(Collectors.joining(", ", "Hello ", ". Goodbye.")); 
		// add Hello as prefix and end with Goodbye
