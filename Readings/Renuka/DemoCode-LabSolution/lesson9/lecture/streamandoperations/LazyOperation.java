...
public class LazyOperation {
	...
	public static void main(String[] args) {
		...
		// functional - though it appears expensive, this also takes only 8 computations
		// for the given example.
		System.out.println(values.stream().filter(LazyOperation::isGreaterThan3).filter(LazyOperation::isEven)
				.map(LazyOperation::doubleIt).findFirst()

				.get());

		// To see the actual computations that took place for both versions, uncomment
		// the three println below.
	}
