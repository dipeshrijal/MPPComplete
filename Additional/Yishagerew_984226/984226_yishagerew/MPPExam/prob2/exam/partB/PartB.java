package MPPExam.prob2.exam.partB;
...
public class PartB {
	...
	//Stating checkoutrecord for param record is optional as shown in comment below
	Function<CheckoutRecord, List<CheckoutRecordEntry>> entryFun = (CheckoutRecord record) -> record.getCheckoutEntries();
	//Function<CheckoutRecord, List<CheckoutRecordEntry>> entryFun = record -> record.getCheckoutEntries();

	//provide a method reference and the type of method reference
	//TYPE:Class::instanceMethod
	Function<CheckoutRecord, List<CheckoutRecordEntry>> entryMethodRef = CheckoutRecord::getCheckoutEntries;
	  
	
	// provide an inner class that behaves the same way as the lambda
	//class My--- implements --- { }
	class EntryExtractor implements Function<CheckoutRecord, List<CheckoutRecordEntry>>{
		@Override
		public List<CheckoutRecordEntry> apply(CheckoutRecord checkoutRecordEntry) {
			return checkoutRecordEntry.getCheckoutEntries();
		}
	}
	...
	public void evaluator() {
		...
		System.out.println(entryFun.apply(record)); //using lambda expression
		System.out.println("Method reference");
		System.out.println(entryMethodRef.apply(record));
		System.out.println("Inner class");
		System.out.println(new EntryExtractor().apply(record));
