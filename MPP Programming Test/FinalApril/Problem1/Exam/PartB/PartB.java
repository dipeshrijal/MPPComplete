...
public class PartB {
	...
	Function<CheckoutRecord, List<CheckoutRecordEntry>> getAllRecords = (CheckoutRecord record) -> record.getCheckoutEntries();
	
	
	//provide a method reference and the type of method reference
	//TYPE: 
	Function<CheckoutRecord, List<CheckoutRecordEntry>> getAllRecords2 = CheckoutRecord::getCheckoutEntries;
	
	// provide an inner class that behaves the same way as the lambda
	//class My--- implements --- { }
	class getAllRecords3 implements Function<CheckoutRecord, List<CheckoutRecordEntry>>{
		@Override
		public List<CheckoutRecordEntry> apply(CheckoutRecord r) {
			return r.getCheckoutEntries();
		}
	}
	...
	public void evaluator() {
		...
		System.out.println("Lambda: " + getAllRecords.apply(record));
		System.out.println("Methos Reference: " + getAllRecords2.apply(record));
		System.out.println("Inner Class: " + (new getAllRecords3()).apply(record));
	}
